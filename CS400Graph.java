...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path source) {
      ...
      List<T> copyDS = source.dataSequence;
      this.dataSequence = new LinkedList<T>();
      this.dataSequence.addAll(copyDS);
      this.end = source.end;
    }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    // containsVertex(end);
    // containsEdge(start, end);
    // } catch (NullPointerException exp) {
    // throw new NoSuchElementException("Cannot find the shortest path!");
    // }
    if (!(this.containsVertex(start)) || !(this.containsVertex(end))) {
      throw new NoSuchElementException();
    }

    Hashtable<T, Path> paths = new Hashtable<>();
    for (Vertex v : this.vertices.values()) {
      paths.put(v.data, new Path(v));
      paths.get(v.data).distance = Integer.MAX_VALUE; // Max integer, get(v) or get(v.data)???
    }

    paths.get(start).distance = 0;
    List<T> visited = new LinkedList<T>();
    PriorityQueue<Path> pq = new PriorityQueue<Path>();
    pq.add(paths.get(start));

    while (!pq.isEmpty()) {
      Path shortP = pq.remove();
      visited.add(shortP.end.data);

      if (shortP.end.data.equals(end)) {
        return shortP;
      }

      for (Edge e : shortP.end.edgesLeaving) {
        Path np = new Path(shortP);
        int tempW = e.weight + np.distance;
        np.extend(e);
        if (tempW < paths.get(e.target.data).distance) {
          pq.add(np);
        }
      }
    }
